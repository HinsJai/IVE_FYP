...
var (
	...
	db               *surrealdb.DB
	config           WebConfig
	// frame_tim_conn   *grpc.ClientConn
	// box_tim_conn     *grpc.ClientConn
...
// func setup_tim_clients() {
// 	var err error
// 	frame_tim_conn, err = grpc.Dial(config.cam.tim_url, grpc.WithTransportCredentials(insecure.NewCredentials()), grpc.WithBlock())
// 	if err != nil {
// 		log.Fatalf("did not connect to FrameGetter: %v", err)
// 	}
// 	frame_tim_client = pb.NewTimAnalysisClient(frame_tim_conn)
// 	box_tim_conn, err = grpc.Dial(config.server.tim_url, grpc.WithTransportCredentials(insecure.NewCredentials()), grpc.WithBlock())
// 	if err != nil {
// 		log.Fatalf("did not connect to BoxGetter: %v", err)
// 	}
// 	box_tim_client = pb.NewTimAnalysisClient(box_tim_conn)
// }

func setup_clients() {
	var err error
	frame_conn, err = grpc.Dial(config.cam.tim_url, grpc.WithTransportCredentials(insecure.NewCredentials()), grpc.WithBlock())
	if err != nil {
		log.Fatalf("did not connect to FrameGetter: %v", err)
	}
	frame_client = pb.NewAnalysisClient(frame_conn)
	box_conn, err = grpc.Dial(config.server.tim_url, grpc.WithTransportCredentials(insecure.NewCredentials()), grpc.WithBlock())
	if err != nil {
		log.Fatalf("did not connect to BoxGetter: %v", err)
	}
...
// func get_tim_image(c *fiber.Ctx) error {
...
// func get_tim_image_data(wg *sync.WaitGroup) {
// 	defer wg.Done()
// 	imageStream, err := frame_tim_client.GetImage(context.Background(), &pb.Empty{})
// 	if err != nil {
...
// func get_tim_box_data(wg *sync.WaitGroup) {
// 	defer wg.Done()
// 	stream, err := box_tim_client.Analysis(context.Background(), &pb.Empty{})
// 	if err != nil {
...
func main() {
	...
	// setup_tim_clients()
	setup_clients()
	// defer frame_tim_conn.Close()
	// defer box_tim_conn.Close()
	...
	// go get_tim_image_data(&wg)
	// go get_tim_box_data(&wg)
